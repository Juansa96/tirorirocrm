-- ══════════════════════════════════════════════════════════════════════════
-- Tanda 2:
--  · Refuerza la numeración automática de pedidos (idempotente): garantiza la
--    columna, la secuencia y el trigger aunque la migración anterior no se
--    hubiera aplicado. No renumera lo ya numerado.
--  · Privacidad de apellidos SIN flag: cualquier tapicero ve solo la inicial
--    del apellido, SIEMPRE, forzado en la consulta. Se añade
--    panel_cliente_nombres(): el nombre completo del cliente NO se devuelve a un
--    tapicero; la función (SECURITY DEFINER) devuelve el nombre ya recortado.
-- ══════════════════════════════════════════════════════════════════════════

-- ── 1) Numeración automática (idempotente) ───────────────────────────────────
...
-- Backfill correlativo por antigüedad SOLO de los que aún no tienen número
-- (activos/en producción primero; el trigger numera el resto de nuevos).
WITH sin_numero AS (
...
-- ── 2) Privacidad de apellidos (sin flag, siempre para tapiceros) ─────────────
