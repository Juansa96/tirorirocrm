...
import { identificadorConversacion, tiempoRelativo } from "@/lib/whatsapp/types";
import { canalDe, CANAL_LABEL } from "@/lib/whatsapp/canales";
import { CanalIcono } from "@/components/whatsapp/CanalIcono";

// Sección "Mensajes" de la ficha del cliente (WhatsApp, Instagram y email):
// si ha escrito por varios canales se ve el recorrido. Resumen de cada chat enlazado,
...
export function WhatsappCliente({ leadId }: { leadId: string }) {
  ...
  // Recorrido por canales, del primer mensaje al último.
  const porFecha = [...convs].sort((a, b) => (a.createdAt || "").localeCompare(b.createdAt || ""));
  const canales = [...new Set(porFecha.map((c) => canalDe(c.telefono)))];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center gap-2">
        ...
        {canales.length > 1 && (
          <span className="ml-auto inline-flex items-center gap-1 text-xs text-slate-500" title="Canales por los que ha escrito">
            {canales.map((c, i) => (
              <span key={c} className="inline-flex items-center gap-1">{i > 0 && <span className="text-slate-300">→</span>}<CanalIcono canal={c} className="h-3 w-3" /> {CANAL_LABEL[c]}</span>
            ))}
          </span>
        )}
      </div>
          ...
          return (
            <div key={c.id} className="space-y-2">
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <CanalIcono canal={canalDe(c.telefono)} className="h-3.5 w-3.5" />
                <span>{CANAL_LABEL[canalDe(c.telefono)]} · {identificadorConversacion(c)}</span>
