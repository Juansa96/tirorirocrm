...
import { ArrowLeft, Mail, Phone, MapPin, Package, Plus, History } from "lucide-react";
import { useStore, actions } from "@/lib/store";
import {
  ETAPAS, ETAPA_COLORS, VENDEDORES, vendorName, type Etapa,
} from "@/lib/types";
...
function ClienteDetalle() {
  ...
  const { leads, tareas, audit } = useStore();
  ...
  const leadTareas = tareas.filter((t) => t.leadId === lead.id).sort((a, b) => a.fecha.localeCompare(b.fecha));
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => navigate({ to: "/clientes" })} className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50">
            Cerrar
          </button>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
          ...
          <div className="space-y-3 text-sm">
            <InfoRow icon={Mail} label="Email">
              {editing ? (
                <input value={lead.email} onChange={(e) => actions.updateLead(lead.id, { email: e.target.value })} className="w-full rounded border border-slate-200 px-2 py-1" />
              ) : (lead.email || <span className="text-slate-400">—</span>)}
            </InfoRow>
            <InfoRow icon={Phone} label="Teléfono">
              {editing ? (
                <input value={lead.telefono} onChange={(e) => actions.updateLead(lead.id, { telefono: e.target.value })} className="w-full rounded border border-slate-200 px-2 py-1" />
              ) : (lead.telefono || <span className="text-slate-400">—</span>)}
            </InfoRow>
            <InfoRow icon={MapPin} label="Ciudad">
              {editing ? (
                <input value={lead.ciudad} onChange={(e) => actions.updateLead(lead.id, { ciudad: e.target.value })} className="w-full rounded border border-slate-200 px-2 py-1" />
              ) : (lead.ciudad || <span className="text-slate-400">—</span>)}
            </InfoRow>
            <InfoRow icon={Package} label="Producto">
              {editing ? (
                <input value={lead.producto} onChange={(e) => actions.updateLead(lead.id, { producto: e.target.value })} className="w-full rounded border border-slate-200 px-2 py-1" />
              ) : lead.producto}
            </InfoRow>
            {editing && (
              <div>
                ...
                <select
                  value={lead.vendedor}
                  onChange={(e) => actions.updateLead(lead.id, { vendedor: e.target.value })}
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 rounded-lg border border-dashed border-slate-300 p-3 md:grid-cols-[1fr_auto_auto]">
          <input placeholder="Nueva tarea…" value={nuevaTarea.descripcion} onChange={(e) => setNuevaTarea({ ...nuevaTarea, descripcion: e.target.value })} className="rounded border border-slate-200 px-3 py-2 text-sm" />
          <input type="date" value={nuevaTarea.fecha} onChange={(e) => setNuevaTarea({ ...nuevaTarea, fecha: e.target.value })} className="rounded border border-slate-200 px-3 py-2 text-sm" />
          <button onClick={addTask} className="inline-flex items-center justify-center gap-1 rounded-lg bg-[#1a1f36] px-3 py-2 text-sm font-medium text-white hover:bg-[#2a2f46]">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
        ...
        {leadAudit.length === 0 ? (
          ...
          <ul className="space-y-2 text-sm">
            {leadAudit.map((a) => {
              const usuario = a.usuario ? vendorName(a.usuario) : "Sistema";
              const campo = FIELD_LABELS[a.campo] ?? a.campo;
              return (
                <li key={a.id} className="flex flex-wrap items-baseline gap-x-1 gap-y-0.5 border-b border-slate-100 pb-2 last:border-0">
                  <span className="font-medium text-slate-900">{usuario}</span>
                  <span className="text-slate-600">cambió {campo} de</span>
                  ...
                  <span className="ml-auto text-xs text-slate-400">{formatAuditDateTime(a.createdAt)}</span>
                </li>
              );
            })}
          </ul>
