...
import { VENDEDORES, ETAPAS, type Vendedor, type Etapa } from "@/lib/types";
import { todayISO } from "@/lib/format";

export const Route = createFileRoute("/clientes/nuevo")({
  head: () => ({
    meta: [{ title: "Nuevo Lead — TiroCRM" }],
  }),
  component: NuevoLead,
});
    ...
    <div className="mx-auto max-w-2xl space-y-4">
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={form.vendedor}
              onChange={(e) =>
                setForm({ ...form, vendedor: e.target.value as Vendedor })
              }
              className={inputCls}
            >
              {VENDEDORES.map((v) => (
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button
            type="submit"
            className="rounded-lg bg-[#1a1f36] px-4 py-2 text-sm font-medium text-white hover:bg-[#2a2f46]"
          >
            Crear lead
          </button>
        </div>
