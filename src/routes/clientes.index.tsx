...
export const Route = createFileRoute("/clientes/")({
  head: () => ({
    meta: [
      { title: "Clientes — TiroCRM" },
      { name: "description", content: "Listado de clientes y leads" },
    ],
  }),
...
function ClientesList() {
  ...
  const productos = useMemo(
    () => Array.from(new Set(leads.map((l) => l.producto).filter(Boolean))),
    [leads],
  );
  const ciudades = useMemo(
    () => Array.from(new Set(leads.map((l) => l.ciudad).filter(Boolean))),
    [leads],
  );

  const filtered = leads.filter((l) => {
    const ql = q.toLowerCase();
    if (q && !l.nombre.toLowerCase().includes(ql) && !l.email.toLowerCase().includes(ql))
      return false;
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-medium">
                    {l.valor > 0 ? formatCurrency(l.valor) : <span className="text-slate-400">—</span>}
                  </td>
                  <td className="px-4 py-3 text-slate-600">{l.ciudad}</td>
                  <td className="px-4 py-3 text-slate-600">
                    {next ? dateLabel(next.fecha) : <span className="text-slate-400">—</span>}
                  </td>
